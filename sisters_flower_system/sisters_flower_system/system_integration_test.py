...
"""
姐妹花销售系统 - 全面集成测试套件
System Integration Test Suite for Sisters Flower System
...
class MockPsutil:
    ...
    def get_memory_info():
        ...
        try:
            ...
            memory_mb = usage.ru_maxrss / 1024  # KB转MB (Linux) 或 字节转KB (macOS)
            
            # 在某些系统上需要除以1024
            if memory_mb > 10000:  # 可能是字节单位
                memory_mb = memory_mb / 1024
                
            return {'rss': memory_mb * 1024 * 1024}  # 转换为字节
        except:
            return {'rss': 500 * 1024 * 1024}  # 默认500MB
...
class IntegrationTestRunner:
    ...
    def test_system_import(self):
        ...
        try:
            ...
            # 尝试导入系统模块（不执行main函数）
            with patch('sys.argv', ['enhanced_sales_system.py']):  # 模拟命令行参数
                # 导入主模块
    ...
    def test_database_operations(self):
        ...
        try:
            ...
            try:
                ...
                cursor.execute("UPDATE products SET stock = -1 WHERE barcode = 'TX001'")  # 这会失败
                conn.commit()
            except:
                conn.rollback()
            ...
            assert rollback_count == 0, "事务回滚失败"
    ...
    def test_inventory_module(self):
        ...
        try:
            ...
            # 测试条码扫描功能
            test_barcode = '123456789001'  # 测试数据库中的条码
            inventory_module.process_barcode(test_barcode)  # 不应该抛出异常
        ...
        try:
            ...
            try:
                with patch('sqlite3.connect', side_effect=sqlite3.Error("测试数据库错误")):
                    from enhanced_sales_system import DataAnalysisModule
                    analysis = DataAnalysisModule(Mock(), invalid_db_path)
            ...
            # 3. 测试输入验证
            from enhanced_sales_system import GoalManagementModule
            goal_module = GoalManagementModule(Mock(), self.test_db_path)
    ...
    def test_performance_and_memory(self):
        ...
        try:
            ...
            # 测试模块加载性能
            start_time = time.time()
            with patch('tkinter.Tk.withdraw'):  # 防止GUI窗口显示
                from enhanced_sales_system import DataAnalysisModule, GoalManagementModule, SettingsModule
                
                analysis = DataAnalysisModule(Mock(), self.test_db_path)
                goals = GoalManagementModule(Mock(), self.test_db_path)
                settings = SettingsModule(Mock(), self.test_db_path)
    ...
    def test_ui_components(self):
        ...
        try:
            ...
            combo.current(0)
            assert combo_var.get() == "选项1", "下拉框选择失败"
            ...
            try:
                ...
                # 创建模拟对话框（不显示）
                with patch('tkinter.Toplevel.wait_window'):  # 防止阻塞
                    with patch('enhanced_sales_system.messagebox.showerror'):  # 防止显示错误框
                        dialog = GoalDialog(Mock(), "测试对话框", {})
...
def main():
    """主函数"""
    print("🌸 姐妹花销售系统 - 全面集成测试套件")
    print("=" * 60)
    print("本测试套件将对系统进行全面功能验证")
    print("包括：启动流程、模块加载、数据库操作、UI组件、错误处理等")
    print()
    ...
    # 创建测试运行器
    runner = IntegrationTestRunner(target_file)
